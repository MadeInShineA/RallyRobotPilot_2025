...
def _(
    ...
    def print_classification_reports(y_true, y_pred):
        ...
        print(classification_report(y_true[:, 0], y_pred[:, 0], target_names=["0", "1"]))
        print("Back Classification Report:")
        print(classification_report(y_true[:, 1], y_pred[:, 1], target_names=["0", "1"]))
        print("Left Classification Report:")
        print(classification_report(y_true[:, 2], y_pred[:, 2], target_names=["0", "1"]))
        print("Right Classification Report:")
        print(classification_report(y_true[:, 3], y_pred[:, 3], target_names=["0", "1"]))
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_train_np), 1):
        ...
        for arch_name, hidden_layers, activation in architectures:
            ...
            for epoch in range(1, epochs + 1):
                ...
                train_forward_acc, train_back_acc, train_left_acc, train_right_acc = compute_accuracy(y_true_train, y_pred_train)
                ...
                train_forward_f1 = f1_score(y_true_train[:, 0], y_pred_train[:, 0], average="binary")
                train_back_f1 = f1_score(y_true_train[:, 1], y_pred_train[:, 1], average="binary")
                train_left_f1 = f1_score(y_true_train[:, 2], y_pred_train[:, 2], average="binary")
                train_right_f1 = f1_score(y_true_train[:, 3], y_pred_train[:, 3], average="binary")

                y_true_val, y_pred_val = evaluate(model, val_loader)
                val_forward_acc, val_back_acc, val_left_acc, val_right_acc = compute_accuracy(y_true_val, y_pred_val)
                ...
                val_forward_f1 = f1_score(y_true_val[:, 0], y_pred_val[:, 0], average="binary")
                val_back_f1 = f1_score(y_true_val[:, 1], y_pred_val[:, 1], average="binary")
                val_left_f1 = f1_score(y_true_val[:, 2], y_pred_val[:, 2], average="binary")
                val_right_f1 = f1_score(y_true_val[:, 3], y_pred_val[:, 3], average="binary")
                ...
                if epoch == epochs:
                    ...
                    forward_f1 = f1_score(y_true_val[:, 0], y_pred_val[:, 0], average="binary")
                    back_f1 = f1_score(y_true_val[:, 1], y_pred_val[:, 1], average="binary")
                    left_f1 = f1_score(y_true_val[:, 2], y_pred_val[:, 2], average="binary")
                    right_f1 = f1_score(y_true_val[:, 3], y_pred_val[:, 3], average="binary")
...
def _(architectures, metrics_per_arch, plt):
    ...
    forward_accuracies = [[m["final_forward_acc"] for m in metrics_per_arch[arch_name]] for arch_name in arch_names]
    back_accuracies = [[m["final_back_acc"] for m in metrics_per_arch[arch_name]] for arch_name in arch_names]
    left_accuracies = [[m["final_left_acc"] for m in metrics_per_arch[arch_name]] for arch_name in arch_names]
    right_accuracies = [[m["final_right_acc"] for m in metrics_per_arch[arch_name]] for arch_name in arch_names]
    forward_f1s = [[m["forward_f1"] for m in metrics_per_arch[arch_name]] for arch_name in arch_names]
    back_f1s = [[m["back_f1"] for m in metrics_per_arch[arch_name]] for arch_name in arch_names]
    left_f1s = [[m["left_f1"] for m in metrics_per_arch[arch_name]] for arch_name in arch_names]
    right_f1s = [[m["right_f1"] for m in metrics_per_arch[arch_name]] for arch_name in arch_names]
    ...
    # Create labels
    labels = ([f"{name} (Forward)" for name in arch_names] + [f"{name} (Back)" for name in arch_names] +
              [f"{name} (Left)" for name in arch_names] + [f"{name} (Right)" for name in arch_names])

    # Colors
    colors = ['green'] * len(arch_names) + ['red'] * len(arch_names) + ['blue'] * len(arch_names) + ['orange'] * len(arch_names)
    ...
    plt.figlegend(handles=legend_elements, loc='upper center', ncol=4, bbox_to_anchor=(0.5, 0.96))
...
def _(best_arch_name, epochs, metrics_per_arch, np, plt):
    ...
    for _fold, metrics in enumerate(best_metrics_list):
        ...
        plt.subplot(2, 4, 2)
        plt.plot(epochs_range, train_forward_error, label="Forward Train Error", color="green", linestyle='-')
        plt.plot(epochs_range, val_forward_error, label="Forward Val Error", color="green", linestyle='--')
        plt.xlabel("Epoch")
        ...
        # --- Plot 3: Back Errors ---
        plt.subplot(2, 4, 3)
        plt.plot(epochs_range, train_back_error, label="Back Train Error", color="red", linestyle='-')
        plt.plot(epochs_range, val_back_error, label="Back Val Error", color="red", linestyle='--')
        plt.xlabel("Epoch")
        plt.ylabel("Error (1 - Accuracy)")
        ...
        # --- Plot 4: Left Errors ---
        plt.subplot(2, 4, 4)
        plt.plot(epochs_range, train_left_error, label="Left Train Error", color="blue", linestyle='-')
        plt.plot(epochs_range, val_left_error, label="Left Val Error", color="blue", linestyle='--')
        plt.xlabel("Epoch")
        plt.ylabel("Error (1 - Accuracy)")
...
def _(
    ...
    print("\nForward Classification Report:")
    print(classification_report(y_true_test[:, 0], y_pred_test[:, 0], target_names=["0", "1"]))
    print("Back Classification Report:")
    print(classification_report(y_true_test[:, 1], y_pred_test[:, 1], target_names=["0", "1"]))
    print("Left Classification Report:")
    print(classification_report(y_true_test[:, 2], y_pred_test[:, 2], target_names=["0", "1"]))
    print("Right Classification Report:")
    print(classification_report(y_true_test[:, 3], y_pred_test[:, 3], target_names=["0", "1"]))
